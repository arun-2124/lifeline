...
export const onDeliveryStatusChanged = functions.firestore
  ...
  .onUpdate(async (change, context) => {
    ...
    Logger.info(`[DeliveryStatusTrigger] Delivery ${deliveryId} status changed: ${oldStatus} ──► ${newStatus}`);
    ...
    try {
      ...
      if (newStatus === 'Pickup Started') {
        ...
        Logger.info(`[DeliveryStatusTrigger] Rescue completed & carbon savings calculated for donation ${donationId}`);
      }

    } catch (error) {
      Logger.error(`[DeliveryStatusTrigger] Error updating delivery status for ${deliveryId}`, error);
    }
  });
